...
# === CONFIGURATION ===
folder_path = r"D:\extracted data from JSON file ISI\extracted data\data_processing_denoised\sensor_data_cleaned_denoised\sensor_data_cleaned"
vibration_axes = ['x_smooth', 'y_smooth', 'z_smooth']
...
for root, dirs, files in os.walk(folder_path):
    for file in files:
        if file.endswith('_denoised.xlsx') and not file.startswith('~$'):
...
with open(error_log_path, "w") as log_file:
    for file_path in tqdm(excel_files, desc="Processing Excel files"):
        try:
            ...
            # Locate required columns (even if shuffled)
            col_map = {}
            ...
            # Preserve full datetime precision
